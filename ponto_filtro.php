...
function validaDadosPonto($registro)
{
    ...
    //Facrof
    $registro->valor_relacao_ponto = str_replace(".", "", $registro->valor_relacao_ponto);
    $registro->valor_relacao_ponto = str_replace(",", ".", $registro->valor_relacao_ponto);
    if (!filter_var($registro->valor_relacao_ponto, FILTER_VALIDATE_FLOAT)) {
        $erros["valor_relacao_ponto"] =  "Valor Relacao: Campo vazio e ou informação inválida!";
    }

    $registro->duracao_ponto = str_replace(".", "", $registro->duracao_ponto);
    $registro->duracao_ponto = str_replace(",", ".", $registro->duracao_ponto);
    if (!filter_var($registro->duracao_ponto, FILTER_VALIDATE_FLOAT)) {
        $erros["duracao_ponto"] =  "Valor Duracao: Campo vazio e ou informação inválida!";
    }
